...
int main(void)
{
    ...
    cv::createTrackbar("ROI_TOP_HEIGHT", "ROIFrame", &ROI_TOP_HEIGHT, frame.rows);
    cv::createTrackbar("ROI_BOTTOM_HEIGHT", "ROIFrame", &ROI_BOTTOM_HEIGHT, frame.rows);
    cv::createTrackbar("ROI_TOP_WIDTH", "ROIFrame", &ROI_TOP_WIDTH, frame.cols);
    cv::createTrackbar("ROI_BOTTOM_WIDTH", "ROIFrame", &ROI_BOTTOM_WIDTH, frame.cols);
    ...
    while (1)
    {
        ...
        for (i = horizontalCount = 0; i < houghProbabilisticLines.size(); i++)
        {
            ...
            {
                cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(255, 0, 0), 1, cv::LINE_AA);
                continue;
            }
            ...
            if (std::fabs(dy_over_dx) < HORIZONTAL_GRADIENT_THRESHOLD)
            {
                ...
                {
                    horizontalCount++;
                    cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(0, 255, 255), 1, cv::LINE_AA);
                }
            ...
            else
            {
                ...
                if ((houghProbabilisticLines[i][1] < leftY1) && (houghProbabilisticLines[i][3] < leftY2))
                {
                    if (dy_over_dx < 0)
                    {
                        ...
                        cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
                    }
                    else
                        cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(255, 0, 0), 1, cv::LINE_AA);
                }
                else if ((houghProbabilisticLines[i][1] < rightY1) && (houghProbabilisticLines[i][3] < rightY2))
                {
                    if (dy_over_dx > 0)
                    {
                        ...
                        cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(0, 0, 255), 1, cv::LINE_AA);
                    }
                    else
                        cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(255, 0, 0), 1, cv::LINE_AA);
                }
                else
                {
                    ...
                    cv::line(houghFrame, cv::Point(houghProbabilisticLines[i][0], houghProbabilisticLines[i][1]), cv::Point(houghProbabilisticLines[i][2], houghProbabilisticLines[i][3]), cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
                }
            }
        }
