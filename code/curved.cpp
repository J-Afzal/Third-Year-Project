...
int main(void)
{
	...
	int currentTurningAdjusted = 100;
	const double TOP = 500;
	const double BOTTOM = 100;



	// Debug Windows and trackbars
	cv::namedWindow("ROIFrame", cv::WINDOW_AUTOSIZE);
	cv::createTrackbar("currentTurning", "ROIFrame", &currentTurningAdjusted, 200);
	...
	while (1)
	{
		...
		double currentTurning = (currentTurningAdjusted - 100) / 100.;
		if (currentTurningAdjusted == 200)
			currentTurning = 1;

		// left
		if (currentTurning < 0)
		{
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 - ROI_TOP_WIDTH / 2 + currentTurning * TOP, ROI_TOP_HEIGHT));
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 + ROI_TOP_WIDTH / 2 + currentTurning * TOP, ROI_TOP_HEIGHT));
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 + ROI_BOTTOM_WIDTH / 2, ROI_BOTTOM_HEIGHT));
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 - ROI_BOTTOM_WIDTH / 2 + currentTurning * BOTTOM, ROI_BOTTOM_HEIGHT));
		}
		//right
		else if (currentTurning > 0)
		{
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 - ROI_TOP_WIDTH / 2 + currentTurning * TOP, ROI_TOP_HEIGHT));
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 + ROI_TOP_WIDTH / 2 + currentTurning * TOP, ROI_TOP_HEIGHT));
			maskDimensions.push_back(cv::Point(VIDEO_WIDTH / 2 + ROI_BOTTOM_WIDTH / 2 + currentTurning * BOTTOM, ROI_BOTTOM_HEIGHT));
