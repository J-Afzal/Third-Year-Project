...
int main(void)
{
	...
	// CUDA objects
	cv::cuda::GpuMat GpuFrame, GpuBlankFrame, GpuROIFrame, GpuCannyFrame, GpuHoughLines;

	cv::Ptr<cv::cuda::CannyEdgeDetector> cannyDetector = cv::cuda::createCannyEdgeDetector(CANNY_LOWER_THRESHOLD, CANNY_UPPER_THRESHOLD, 3, true);
	//cv::Ptr<cv::cuda::HoughSegmentDetector> houghDetector = cv::cuda::createHoughSegmentDetector(10, CV_PI / 18, HOUGHP_MIN_LINE_LENGTH, HOUGHP_MAX_LINE_GAP, 4096);
	//cv::Ptr<cv::cuda::HoughLinesDetector> houghDetector = cv::cuda::createHoughLinesDetector(1, CV_PI / 180, 0);
	...
	while (1)
	{
		...
		//houghDetector->detect(GpuCannyFrame, GpuHoughLines);

		//houghLines.resize(GpuHoughLines.cols);
		//cv::Mat temp_Mat(1, GpuHoughLines.cols, CV_32SC4, &houghLines[0]);
		//GpuHoughLines.download(temp_Mat);
		...
		case 0: // Within lanes
		{
			...
			GpuFrame.upload(frame);
			GpuBlankFrame.upload(blankFrame);
			cv::cuda::add(GpuFrame, GpuBlankFrame, GpuFrame);
